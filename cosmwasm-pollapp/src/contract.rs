...
const CONTRACT_NAME: &str = "crates.io:pollapp";
...
mod tests {
    ...
    use cosmwasm_std::{attr, from_binary}; // helper to construct an attribute ("action", "instantiate") // mock functions to mock an environment, messagesInfo, dependencies // our instantiate method
    ...
    #[test]
    fn test_inctantiate() {
        // mock the dependencies, must be mutable so we can pass it as a mutable, empty vector means out contract has no balance
        ...
        // Create a message where we (the sender) will be the admin
    ...
    // Unsuccessful poll creation (more than 10 options).
    // Successful voting on a poll we have not already voted for.
    // Successful voting a poll we have already voted for.
    // Unsuccessful voting a poll, (invalid option).
    ...
    fn test_execute_create_poll_valid() {
        ...
        let msg = ExecuteMsg::CreatePoll {
            ...
            question: "What's your favorite coin?".to_string(),
            options: vec![
    ...
    fn test_execute_create_poll_invalid() {
        ...
        let msg = ExecuteMsg::CreatePoll {
            poll_id: "some_id".to_string(),
            question: "whats your favorite number?".to_string(),
            options: vec![
                "1".to_string(),
                "2".to_string(),
                "3".to_string(),
    ...
    fn test_execute_vote_invalid() {
        ...
        // Create the vote, some_id poll is not create yet.
        let msg = ExecuteMsg::Vote {
            poll_id: "some_id".to_string(),
            vote: "Osmosis".to_string(),
        };
    ...
    fn test_query_all_polls() {
        ...
        // Instantiate the contract
        let msg = InstantiateMsg { admin: None };
        let _r = instantiate(deps.as_mut(), env.clone(), info.clone(), msg).unwrap();
    ...
    fn test_query_poll() {
        ...
        // query for the polls that exists
        ...
        // Query for the poll that doesnot not exists
        let msg = QueryMsg::Poll {
            poll_id: "some_id_doesnot_exist".to_string(),
        };
        let bin = query(deps.as_ref(), env.clone(), msg).unwrap();
        let res: PollResponse = from_binary(&bin).unwrap();
