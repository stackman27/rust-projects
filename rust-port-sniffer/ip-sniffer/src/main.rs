...
#[bpaf(options)] // this is command line parser
struct Arguments {
...
async fn scan(tx: Sender<u16>, start_port: u16, addr: IpAddr) {
    ...
    match TcpStream::connect(format!("{}:{}", addr, start_port)).await {
        // If the connection is successful, print out a . and the npass the port through the channel
        Ok(_) => {
            print!(".");
...
#[tokio::main] // allows async function
async fn main() {
    // collect the arguments
    let opts = arguments().run();
    // Initialize the channel
    let (tx, rx) = channel();
    // Iterate through all of the ports(based on user inputs) so that we can spawn a single task for each
    ...
    // Create the vector for all the outputs.
    let mut out = vec![];
    // Drop the tx clones.
    drop(tx);

    // Wait for all the outputs to finish and push them into the vector.
    for p in rx {
        out.push(p)
    }
    println!("");
